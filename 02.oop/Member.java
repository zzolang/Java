...
// => 메소드를 호출한 인스턴스 정보를 전달받아 자동으로 저장
// => 메소드에서 인스턴스의 필드를 명확히 구분하기 위해 사용
// => 메소드에서 this 표현하지 않아도 기본적으로 this가 사용되어 표현

//this 키워드를 사용하는 경우
//1.메소드의 지역변수(매개변수)와 필드의 이름이 같은 경우 this를 사용하여 필드를 표현
// => this를 사용하지 않으면 기본적으로 지역변수로 처리
//2.생성자에서 다른 생성자를 호출하여 초기화 작업을 하고자 할 경우 this 사용
//=> 생성자 호출 명령 전에 다른 명령을 선언할 경우 에러 발생
//3.메소드에서 호출한 인스턴스를 표현하기 위해 사용 => 이벤트 처리, 다중 스레드...
...
public class Member {
	...
	//생성자(Constructor)를 선언하지 않을 경우 기본 생성자가 내부적으로 기본 생성자가 제공 
	
	//생성자 이름은 클래스명과 동일하게 선언하며 반환형을 미선언
	// => 메소드 오버로드 기능에 의해 여러 개의 생성자 선언 가능
	
	//매개변수가 없는 생성자 선언 - 기본 생성자(Default Constructor) 
	// => 초기화 작업 미구현
	// => 선언하지 않을 경우 클래스의 상속성에 문제가 발생하므로 반드시 선언
	//[Ctrl]+[Space] >> Constructor 선택 >> 엔터(클릭)
	public Member() {
		// this 키워드를 이용해 다른 생성사 호출
		this("NoId","NoName","NoEmail");
	}
	
	//매개변수가 선언된 생성자 선언
	// => 인스턴스를 생성시 필드에 원하는 값 저장 - 초기화 작업
