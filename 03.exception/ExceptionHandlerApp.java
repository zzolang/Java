...
// => 예외가 발생되는 명령에 대한 처리 - 예외처리
// => 심각한 예외인 경우 프로그램 실행 전 예외 처리를 하지 않을 경우 에러로 표시
...
public class ExceptionHandlerApp {
	public static void main(String[] args) {
		...
		// => 예외가 발생된 경우 예외 처리 클래스로 인스턴스를 생성
		// => 예외 발생 명령 아래에 존재하는 명령 미실행 - 스레드(프로그램 흐름)가 실행
		try {
			// 배열 요소의 첨자가 범위를 벗어날 경우 ArrayIndexOutOfBoundsException 발생
			...
			System.out.println("[결과]프로그램을 정상적으로 실행하였습니다.");
		} catch (ArrayIndexOutOfBoundsException e) {
			//예외가 발생될 경우 해당 catch 영역의 명령 실행
			// => try 영역에서 발생된 해당 예외 인스턴스를 전달받아 저장하여 예외 처리
			// => 예외처리 :프로그램 사용자에게 메세지를 제공하거나 개발자에게 에러로그를 제공
