...
//키보드로 메세지를 입력받아 접속 사용자에게 전달하는 UDP 프로그램
public class MessageSendApp {
	public static void main(String[] args) throws IOException {
		...
		//DatagramPacket(byte[], int length, InetAddress serverName, int port) 생성자
		DatagramPacket packet=new DatagramPacket(data, data.length, address, 5000);
		
		//DatagramSocket.send(DatagramPacket packet) : 패킷에 저장된 정보를 이용하여 다른 컴퓨터에게 전달하는 메소드
		socket.send(packet);
