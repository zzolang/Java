...
public class TimeClientApp {
	public static void main(String[] args) throws ClassNotFoundException {
		...
		try {
			...
			//socket.getInputStream() : Socket 인스턴스의 입력 스트림을 반환하는 메소드
			InputStream in=socket.getInputStream();
			
			//입력 스트림을 전달받아 객체 읽기가 가능한 스트림으로 확장
			ObjectInputStream stream=new ObjectInputStream(in);
			
			//서버에서 제공한 인스턴스를 반환받아 저장
			Date serverDate=(Date)stream.readObject();
			
			System.out.println("[결과]서버에서 제공된 날짜와 시간 = "+new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(serverDate));
