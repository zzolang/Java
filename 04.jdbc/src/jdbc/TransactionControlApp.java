...
//=> 프로그램 실행시 다양한 예외가 발생될 경우 전달되어 이전에 전달되어 실행된 SQL 명령은 롤백 처리하는 것을 권장
...
public class TransactionControlApp {
	public static void main(String[] args) {
		...
		try {
			...
			// Con.setAutoCommit(boolean autoCommit) : 자동 커밋 기능의 사용 여부를 변경하는 메소드
			// => false : 자동 커밋 기능 비활성화, true : 자동 커밋 기능 활성화(기본)
			con.setAutoCommit(false);

			stmt = con.createStatement();

			String sql = "update student set name ='홍길동' where no=1000";
			int rows = stmt.executeUpdate(sql);
			
			//throw new Exception("나도 알 수 없는 예외입니다.");
		...
		} catch (Exception e) { //프로그램 실행시 발생된 예외 처리
