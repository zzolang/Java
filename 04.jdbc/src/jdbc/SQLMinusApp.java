...
//키보드로 SQL 명령(INSERT,UPDATE,DELETE,SELECT)을 입력받아 DBMS 서버에 전달하여
//실행된 처리결과를 출력하는 JDBC 프로그램
						...
						do {
							for(int i=1;i<=columnCount;i++) {
								...
								if(rsmd.getColumnTypeName(i).equals("DATE")) {
									columnValue=columnValue.substring(0,10);//2000-12-31
								}
								if(columnValue==null) {
									columnValue="    ";
								}
