...
//CREATE TABLE STUDENT(NO NUMBER(4) PRIMARY KEY, NAME VARCHAR2(20), PHONE VARCHAR2(15),ADDRESS VARCHAR2(50), BIRTHDAY DATE);

//STUDENT 테이블에 학생정보를 저장하는 JDBC 프로그램
public class InsertStudenApp {
	public static void main(String[] args)  {
		
		//JDBC 관련 인스턴스를 저장하는 참조변수는 try 영역 외부에서 선언
		//=> finally 영역에서 참조변수 사용 가능
		Connection con=null;
		Statement stmt=null;
		...
		try {
			...
			//OracleDriver 인스턴스를 전달하여 JDBC Driver로 등록
			DriverManager.registerDriver(driver);
			 */
			
			//1. OracleDriver 클래스를 읽어 메모리에 저장
			//=> OracleDriver 클래스를 인스턴스로 생성하여 JDBC 드라이버로 등록
			//JDBC 드라이버 : DriverManager 클래스로 등록된 Driver 인스턴스로
			...
			//=> SQLException 발생 : JDBC 관련된 메소드에서 발생하는 예외
			...
			//=> Statement : 인스턴스를 저장 할 수 있는 참조변수 생성
			//=> SQL 명령을 저장하여 접속된 서버에 전달하기 위한 기능을 제공하는 인스턴스
			stmt=con.createStatement();
			
			//4. Statement 인스턴스에 SQL 명령을 저장하여 접속된 서버에 전달하여 결과를 반환받아 저장
	...
	//		String sql="insert into student values(2000,'임꺽정','010-1234-5678','경기도 수원시','2000-10-05')";
			String sql="insert into student values(3000,'전우치','010-1577-1577','인천시 계양구','2002-03-20')";
