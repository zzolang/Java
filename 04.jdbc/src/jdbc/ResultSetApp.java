...
public class ResultSetApp {
	public static void main(String[] args) throws SQLException{
		...
		//=> 커서위치에 행이 없는 경우 false 반환, 있는 경우 true 반환
		...
		//resultSetType : ResultSet 커서 이동에 관련 속성 변경
		//=> ResultSet.TYPE_FORWARD_ONLY : ResultSet 커서를 아래행으로만 이동
		...
		//ResultSet.beforeFirst() : ResultSet 커서를 BOF 위치의 검색행으로 이동하는 메소드
		rs.beforeFirst();
		...
		//ResultSet.afterLast() : ResultSet 커서를 EOF 위치의 검색행으로 이동하는 메소드
		rs.afterLast();
		...
		//커서 위치의 행을 조작하고자 할경우 SELECT 명령에서 검색대상으로 * 사용 불가능 >> * 사용시 조작 불가능
		sql="select no, name, phone, address, birthday from student order by no";
		rs=stmt.executeQuery(sql);
		
		/*
		//ResultSet 인스턴스에 저장된 검색행에서 2번째 위치에 새로운 행을 삽입하여 테이블에 적용 >> INSERT
		rs.absolute(2);
		
		//Result.moveToInsetRow() : 커서 위치에 새로운 행을 생성하는 메소드
		//=> 기존 행들은 자동으로 뒤로 이동
		rs.moveToInsertRow();
		
		//Result.updateXXX(String columnName, XXX value) : 커서 위치의 행에 컬럼값을 변경하는 메소드
		...
		//ResultSet.insertRow() : 커서 위치의 행으로 테이블에 행을 삽입하는 메소드
		rs.insertRow();
		...
		*/
		...
		/*
		//ResultSet 인스턴스에 저장된 검색결과에서 3번째 위치의 검색행 삭제하여 테이블에 적용 >> DELETE
		rs.absolute(3);
		
		//ResultSet.deleteRow() : 커서 위치의 행으로 테이블에 행을 삭제하는 메소드
		rs.deleteRow();
		*/
		
		//ResultSet 인스턴스에 저장된 검색결과에서 2번째 위치의 검색행에 이름을 [임꺽정]으로 변경하여 테이블에 적용 >> UPDATE
		rs.absolute(2);
		rs.updateString("name", "임꺽정");
		
		//ResultSet.updateRow() : 커서 위치의 행으로 테이블에 행을 변경하는 메소드
		rs.updateRow();
