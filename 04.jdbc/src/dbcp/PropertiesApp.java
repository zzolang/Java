...
public class PropertiesApp {
	public static void main(String[] args) throws IOException {
		...
		//FileInputStream 클래스를 이용하여 입력스트림을 생성
		//=> FileInputStream 클래스를 이용할 경우 파일 경로 표현에 문제 발생
		//FileInputStream in = new FileInputStream("bin/site/itwill/dbcp/user.properties");
	
		//리플렉션 기능을 이용하여 입력스트림 생성
		//=> 클래스.class - 클래스를 메모리에 저장(Clazz : Class 인스턴스)하여 사용
		//Class.getClassLoader () : ClassLoader 인스턴스를 반환하는 메소드
		//ClassLoader.getResourceAsStream(String resource) : 리소스 파일을 읽기 위한
		//입력스트림을 생성하여 반환하는 메소드
