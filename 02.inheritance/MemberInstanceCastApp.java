...
public class MemberInstanceCastApp {
	public static void main(String[] args) {
		...
		// => 부모 인스턴스를 생성하여 인스턴스를 부모 참조변수에 저장 가능
		...
		//객체 형변환을 이용하면 참조변수를 이용하여 자식 인스턴스 메소드에도 접근 가능
		//명시적 객체 형변환(강제 형변환) : 참조변수의 자료형을 자식클래스로 
		//일시적으로 변경하여 자식 인스턴스를 저장되도록 설정 - Cast 연산자 사용
		/*
		MemberEvent temp=(MemberEvent)member4; // 다운캐스팅 (부모가 자식이 됨)
		...
		*/
		...
		//*****묵시적 객체 형변환(자동 형변환) : 자식클래스에서 오버라이드 선언된 메소드는
		//부모클래스의 메소드 대신 자식클래스의 메소드가 호출 되도록 
		//자동으로 참조변수에 저장된 인스턴스가 자식인스턴스로 변환******** 중요함 
		member4.display();
		System.out.println("=================================================");
