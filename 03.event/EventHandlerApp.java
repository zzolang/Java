...
//=> 리스너 인터페이스 : 이벤트가 처리하기 위한 추상메소드가 선언된 인터페이스
//=> 추상메소드를 오버라이드 선언하여 메소드에 이벤트 처리명령을 작성

//3. 이벤트 소스에서 이벤트가 발생될 경우 이벤트 핸들러가 동작되도록 이벤트 등록
//=> Component.addXXXLisner(XXXListener l) : 컴퍼넌트에서 이벤트가 발생될 경우 
//   이벤트 핸들러 인스턴스를 이용하여 이벤트 처리하는 메소드
...
public class EventHandlerApp extends Frame {
	...
	public EventHandlerApp(String title) {
		...
		//=> 시스템(OS)에서 컴퍼넌트의 이벤트에 대한 이벤트 인스턴스 자동 생성
		...
		//=> 이벤트 핸들러 인스턴스의 이벤트 처리 메소드가 자동 호출 
...
class ButtonEventHandler implements ActionListener{
	//Button 컴퍼넌트를 누른 경우 호출되어 실행되는 이벤트 처리 메소드
	@Override
