...
public class DoubleArrayApp {
	public static void main(String[] args) {
		...
		//=> 1차원 배열의 요소 갯수를 다르게 선언 가능 - 가변길이 배열
		temp[0]=new int[3];
		temp[1]=new int[2];
		temp[2]=new int[4];
		...
		*/
		...
		//학생이름과 국어,영어,수학 점수를 입력받아 학생점수 합계, 과목별
		...
		int[][] score= {{80,90,100,0},{76,89,77,0},{78,99,87,0},{98,99,100,0},{88,92,90,0},{0,0,0,0}};
		...
		for(int i=0;i<score.length-1;i++) {
			for(int j=0;j<score[i].length-1;j++) {
				...
				//score[i][score[i][score.length-1] : 학생 점수 합계가 저장된 요소
				score[i][score[i].length-1]+=score[i][j];
				
				//score[score.length-1][j] : 과목별 점수 합계가 저장된 요소
				score[score.length-1][j]+=score[i][j];
				
				//score[score.length-1][score[i][score.length-1] : 전체 점수 합계가 저장된 요소
		...
		//학생이름, 국어, 영어, 수학 점수와 학생 점수 합계, 과목별 점수 합계, 전체 점수 합계 출력
