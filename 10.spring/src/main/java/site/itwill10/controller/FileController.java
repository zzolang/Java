...
public class FileController implements ApplicationContextAware {
	...
	*/
	...
	public ModelAndView fileDownload(@PathVariable int num) {
		...
		//ModelAndView(String viewName,String modelName, Map<String, ?> model) 생성자로 인스턴스 생성 
		//ModelAndView 인스턴스에 저장되는 viewName에는 Spring Bean의 beanName을 설정 
		// => 특정 기능을 제공하는 Spring Bean의 메소드를 호출하여 실행
		// => BeanNameViewResolver 클래스를 이용하여 응답 처리되도록 설정 - servlet-context.xml
		//ModelAndView 인스턴스에 다운로드 기능을 제공하는 Spring Bean을 저장하여 반환
		// => Spring Bean에게 다운로드 파일정보 제공
