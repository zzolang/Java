...
//		=> FileOutputStream(String fileName) : 파일경로를 전달받아 출력스트림을 만들어주는 생성자
...
//		=> 파일이 존재 할 경우 기존 내용 대신 새로운 내용으로 파일에 저장 - 덮어씌우기(OverWrite)발생

//		=> FileOutputStream(String name,boolean append) : 매개변수에 true 값을 
//	       전달할 경우 기존내용뒤에 새로운 내용을 추가하여 파일에 저장 - 이어쓰기
		...
		while(true) {
			...
			if(readByte==-1) break;
			out.write(readByte);//파일 출력스트림에 값을 전달 - 저장
		}
		
//		파일에는 입력스트림과 출력스트림을 하나씩만 생성 가능
//		=> 파일에 생성된 스트림은 반드시 제거
//		FileOutputStream.close() : 파일 출력 스트림 제거하는 메소드
		out.close();
