...
//File 클래스 : 파일정보를 저장하기 위한 클래스 - 파일 관련 기능을 제공
...
public class FileApp {
	public static void main(String[] args) throws IOException {
		...
		//File.exists() : File 인스턴스에 저장된 파일경로에 
		//파일이 없는 경우 false 반환, 존재할 경우 true를 반환하는 메소드
		if(fileOne.exists()) {
				System.out.println("c:\\data 폴더(디렉토리)가 존재합니다.");
		
		} else {
			//FileOne.mkdir() : File 인스턴스에 저장된 파일경로로 디렉토리를 생성
			fileOne.mkdir();
			System.out.println("c:\\data 폴더(디렉토리)를 생성 하였습니다.");
			
		}
		System.out.println("============================================================");
		//File fileTwo= new File("c:\\data\\avc.txt");
		...
		//File(File parent, String child) : 파일경로를 부모(디렉토리)와 자식(파일)으로 전달하여 
		//file 인스턴스를 생성하는 생성자
		//File fileThree=new File(fileOne,"xyz.txt");
		//File(String parent, String child) : 파일경로를 부모(디렉토리)와 자식(파일)으로 전달하여 
		//file 인스턴스를 생성하는 생성자
		File fileThree=new File("c:/data","xyz.txt");
		...
		System.out.println("============================================================");
		//절대경로 표현방법 : 드라이브 또는 최상위 디렉토리를 기준으로 파일경로 표현
		//File fileFour=new File("c:/Users/user/eclipse-workspace/java-3/src");
		...
		//File.isDirectory() : File 인스턴스에 저장된 파일경로의 파일이 
		//디텍토리가 아닌 경우 false 반환하고 디렉토리인 경우 true 를 반환하는 메소드 
		if(fileFive.isDirectory()) {
			...
			for(File file:subFile) {
				//File.isDirectory() : File 인스턴스에 저장된 파일경로의 파일이
				//아닌경우 false 반환하고 파일인 경우 true 반환하는 메소드
				if(file.isFile()) {
